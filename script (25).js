...
(() => {
      ...
      .then(fc => {
        ...
        gnLayer = L.geoJSON(fc, {
          ...
          onEachFeature: (feature, layer) => {
            ...
            layer.on("click", (e) => {
              // --- ADDED: if the user is in "Select Location" mode, a
              // click on a GND polygon should place the feedback marker
              // instead of doing the normal highlight/popup/info-panel
              // behavior (which otherwise silently swallows the click).
              if (pickModeActive) {
  ...
  function enterPickMode() {
    ...
    map.getContainer().classList.add("picking-location"); // ADDED
  }
  function exitPickMode() {
    pickModeActive = false;
    pickBanner.classList.add("hidden");
    map.getContainer().style.cursor = "";
    map.getContainer().classList.remove("picking-location"); // ADDED
  }
  pickBtn.addEventListener("click", enterPickMode);
  cancelPickBtn.addEventListener("click", exitPickMode);

  // --- ADDED: shared picker logic, called from either the map's own
  // click (empty areas) or from a GN polygon's click (colored areas).
  // A polygon's bound popup normally stops the click from ever reaching
  // map.on("click"), which was why picking never worked on a GND.
  ...
  form.addEventListener("submit", e => {
    ...
    if (!CONFIG.APPS_SCRIPT_API_URL || CONFIG.APPS_SCRIPT_API_URL.startsWith("PASTE_")) {
      showFormMsg("Feedback backend isn't configured yet — paste your Apps Script /exec URL into config.js (APPS_SCRIPT_API_URL).", "error");
      return;
    }
    ...
    // CHANGED: submit directly to the Apps Script Web App's doPost handler
    // (writes straight into the Sheet) instead of the Google Form's
    // /formResponse endpoint. This avoids Google Forms' strict
    // multiple-choice validation, which was silently dropping
    // submissions whenever the dropdown text didn't exactly match the
    // Form's predefined choices. Sent as JSON with a text/plain content
    // type so the browser treats it as a "simple request" (no CORS
    // preflight), which Apps Script Web Apps handle without extra setup.
    // Because this is a real (non no-cors) request, we can now read the
    // actual success/failure response instead of assuming it worked.
    fetch(CONFIG.APPS_SCRIPT_API_URL, {
      method: "POST",
      headers: { "Content-Type": "text/plain;charset=utf-8" },
      body: JSON.stringify({ gnDivision: gn, serviceType: service, issueCategory: issue, description, location }),
    })
      .then(r => r.json())
      .then(result => {
        if (result && result.error) {
          showFormMsg(`Could not save feedback: ${result.error}`, "error");
          return;
        }
        ...
        form.reset();
        locationInput.value = "";
        // Reconcile the optimistic entry with the real row shortly after.
        setTimeout(fetchFeedback, 2000);
      })
      .catch(() => {
        showFormMsg("Could not reach the Apps Script backend. Check your connection and try again.", "error");
      })
      .finally(() => {
  ...
  function renderFeedback() {
    ...
    saveFeedbackCache(); // ADDED: keep the cache in sync every time the list changes
