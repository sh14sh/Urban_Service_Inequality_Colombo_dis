...
(() => {
  ...
  // -------------------------------------------------------------
  // COMMUNITY FEEDBACK — OPEN GOOGLE FORM (submissions happen on
  // Google's own form now, so there's no in-page form/location-picker
  // to wire up here anymore; the Sheet + Apps Script + auto-refresh
  // below still pick up every submission automatically).
  // -------------------------------------------------------------
  const openFeedbackFormBtn = document.getElementById("openFeedbackFormBtn");
  if (openFeedbackFormBtn) {
    const viewUrl = CONFIG.GOOGLE_FORM_VIEW_URL
      || (CONFIG.GOOGLE_FORM_ACTION_URL || "").replace("/formResponse", "/viewform");
    openFeedbackFormBtn.href = viewUrl || "#";
  }
