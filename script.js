...
(() => {
  ...
  // -------------------------------------------------------------
  // COMMUNITY FEEDBACK — SUBMIT (writes directly to the Sheet via
  // Apps Script's doPost, bypassing Google Forms' multiple-choice
  // validation, which silently drops a submission whenever the
  // dropdown text doesn't exactly match the Form's predefined choices)
  // -------------------------------------------------------------
  ...
  form.addEventListener("submit", e => {
    ...
    // Sent as JSON with a text/plain content type so the browser treats
    // it as a "simple request" (no CORS preflight), which Apps Script
    // Web Apps handle without extra setup. Because this is a real
    // (non no-cors) request, we can read the actual success/failure
    // response instead of assuming it worked.
