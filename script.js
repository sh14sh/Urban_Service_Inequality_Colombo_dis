    ...
    return fetch("data/GND_layer_n.geojson")
      ...
      .then(fc => {
        ...
        fc.features.forEach(f => {
          ...
          // AI / priority_i are 0–1 decimals; scale to 0–100 so the
          // existing accessibilityColor()/priorityColor() buckets work.
          p.accessibility_index = Math.round(Number(p[FIELD.accessibility]) * 100);
          p.priority_index      = Math.round(Number(p[FIELD.priority]) * 100);
        });
